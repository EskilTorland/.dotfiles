# Generated by Powerlevel10k configuration wizard on 2026-07-30 at 15:54 CEST.
...
() {
  ...
  function prompt_jj() {
    local jj_info
    jj_info=$(jj log --ignore-working-copy --no-graph -r @ -T 'separate(" ", change_id.shortest(8), bookmarks)' 2>/dev/null)
    if [[ $? -eq 0 && -n "$jj_info" ]]; then
      local res="${jj_info}"

      # Tracked file changes (modified/added/deleted)
      local summary
      summary=$(jj diff --ignore-working-copy --summary 2>/dev/null)
      if [[ -n "$summary" ]]; then
        local m a d
        m=$(grep -c '^M ' <<< "$summary")
        a=$(grep -c '^A ' <<< "$summary")
